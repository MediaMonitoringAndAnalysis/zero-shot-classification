...
class MultiStepZeroShotClassifier:
    ...
    def __init__(
        ...
        first_pass_per_sentence: bool = False,
        ...
        self.first_pass_per_sentence = first_pass_per_sentence
        ...
        self.first_pass_classifier = pipeline(
            "zero-shot-classification", model=self.first_pass_model, device=self.device
        )
    ...
    def _one_entry_first_pass_classification(
        ...
        results = []
        for i in tqdm(
            range(0, len(entries), self.batch_size), desc="First pass classification"
        ):
            batch_entries = entries[i : i + self.batch_size]
            batch_results = self.first_pass_classifier(
                batch_entries,
                tags,
    ...
    def _per_sentence_first_pass_classification(
        self,
        entries: List[str],
        tags: List[str],
    ) -> List[Dict[str, float]]:
    ...
    def first_pass_classification(
        ...
        Returns:
            Union[List[List[str]], List[Dict[str, float]]]: List of tags that are relevant to the text

        """
        if self.first_pass_per_sentence:
            raw_results: List[Dict[str, float]] = (
                self._per_sentence_first_pass_classification(entries, tags)
            )
        else:
            raw_results: List[Dict[str, float]] = (
                self._one_entry_first_pass_classification(entries, tags)
            )
            raw_results = [
                {
                    tag: score
                    for tag, score in zip(
                        one_input_result["labels"], one_input_result["scores"]
                    )
                }
                for one_input_result in raw_results
            ]
        ...
        if self.first_pass_threshold is not None:
            ...
            keep_tags = [
                [
                    label
                    for label, scores_list in raw_results[i].items()
    ...
    def second_pass_classification(
        ...
        for i in range(0, n_entries):
            ...
            prompt_content = DETAILED_PROMPT_TEMPLATE.format(
                tags=", ".join(filtered_tags_one_entry)
            )
            all_prompts.append(
                [
    ...
    def __call__(
        self, entries: List[str], tags: List[str]
    ) -> Union[List[List[str]], List[Dict[str, float]]]:
        """
        ...
        # First pass
        first_pass_results = self.first_pass_classification(entries=entries, tags=tags)

        if self.do_second_pass:
            # Second pass
            second_pass_results = self.second_pass_classification(
                entries=entries, filtered_tags=first_pass_results
            )
